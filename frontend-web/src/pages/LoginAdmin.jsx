...
export default function LoginAdmin(){
  ...
  const [username,setU] = useState("admin");     // tu demo
  const [password,setP] = useState("admin123");  // tu demo
  ...
  async function submit(e){
    ...
    try{
      ...
      if(role !== "superadmin"){
        setErr("Pantalla solo para ADMINISTRADOR. Usa el login de docente si no eres admin.");
        return; // NO guardar token
      }
      saveToken(data.token);
      nav("/admin"); // panel admin (otra HU)
    }catch(ex){
